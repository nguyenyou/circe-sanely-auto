...
object SanelyConfiguredCodec:
  ...
  private class ConfiguredCodecDerivation[A: Type](conf: Expr[Configuration])(using val quotes: Quotes):
    ...
    val timer: MacroTimer = MacroTimer.create(Type.show[A], "CfgCodec")
    ...
    private def deriveProductCodec[P: Type, Types: Type, Labels: Type](
      ...
      val fieldsWithDefaults = fields.zipWithIndex.map { case ((label, tpe, enc, dec), idx) =>
        (label, tpe, enc, dec, defaults.lift(idx).flatten)
      }
      ...
      // Encoder arrays
      val encoderExprs = fieldsWithDefaults.map { case (_, tpe, enc, _, _) =>
        tpe match
      ...
      // Decoder arrays
      val decoderExprs = fieldsWithDefaults.map { case (_, tpe, _, dec, _) =>
        tpe match
          case '[t] => '{ ${dec.asInstanceOf[Expr[Decoder[t]]]}.asInstanceOf[Decoder[Any]] }
      }
      val decodersArrayExpr = '{ Array(${Varargs(decoderExprs)}*) }

      val hasDefaultExprs = fieldsWithDefaults.map(f => Expr(f._5.isDefined))
      val hasDefaultArrayExpr = '{ Array(${Varargs(hasDefaultExprs)}*) }

      val defaultExprs = fieldsWithDefaults.map { case (_, _, _, _, defaultOpt) =>
        defaultOpt.getOrElse('{ null })
      }
      val defaultsArrayExpr = '{ Array[Any](${Varargs(defaultExprs)}*) }

      val isOptionExprs = fieldsWithDefaults.map { case (_, tpe, _, _, _) =>
        tpe match
          case '[t] =>
      ...
      '{
        ...
        new Codec.AsObject[P]:
          ...
          def encodeObject(a: P): JsonObject =
            SanelyRuntime.encodeProductFields(a.asInstanceOf[Product], _fieldNames, _encoders)
    ...
    private def deriveProductPair[P: Type, Types: Type, Labels: Type](
      ...
      val enc = '{
        val _names = $fieldLabelsExpr.map($conf.transformMemberNames).toArray
    ...
    private def deriveSumPair[S: Type, Types: Type, Labels: Type](
      ...
      val dec = '{
        new Decoder[S]:
          ...
          private val _isSubTrait = $isSubTraitExpr
          def apply(c: HCursor): Decoder.Result[S] =
            SanelyRuntime.extractSumTypeInfo(c, $conf.discriminator, _transformedLabels, $conf.strictDecoding, ${Expr(TypeRepr.of[S].typeSymbol.name)}, _allTransformedLabels) match
              case Left(err) => Left(err)
              case Right(pair) =>
                SanelyRuntime.decodeSumConfigured(c, pair._1, pair._2, _labels, _decoders, _isSubTrait, $conf.transformConstructorNames, $conf.discriminator, ${Expr(TypeRepr.of[S].typeSymbol.name)})
          override def decodeAccumulating(c: HCursor): Decoder.AccumulatingResult[S] =
            SanelyRuntime.extractSumTypeInfo(c, $conf.discriminator, _transformedLabels, $conf.strictDecoding, ${Expr(TypeRepr.of[S].typeSymbol.name)}, _allTransformedLabels) match
              case Left(err) => cats.data.Validated.invalidNel(err)
              case Right(pair) =>
                SanelyRuntime.decodeSumConfiguredAccumulating(c, pair._1, pair._2, _labels, _decoders, _isSubTrait, $conf.transformConstructorNames, $conf.discriminator, ${Expr(TypeRepr.of[S].typeSymbol.name)})
      }
      (enc, dec)
