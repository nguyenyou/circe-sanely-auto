  ...
  private class EncoderDerivation[A: Type](strict: Boolean)(using val quotes: Quotes):
    ...
    def derive(mirror: Expr[Mirror.Of[A]]): Expr[Encoder.AsObject[A]] =
      if MacroUtils.isRecursiveType(selfType) then
        ...
        deriveInner(mirror, '{ null.asInstanceOf[Encoder.AsObject[A]] })
    ...
    private def resolveOneEncoder[T: Type](
      ...
      // Constructor-level negative cache: skip summonIgnoring for applied types
      // whose constructor was already proven to have no user-provided instances
