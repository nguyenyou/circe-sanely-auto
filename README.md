...
- [ ] **(P1) Mirror-first resolution: skip `summonIgnoring` for derivable types** — `summonIgnoring` is 51% of macro time (1031ms/2028ms, 660 calls, avg 1.56ms). Most calls fail and fall through to `Mirror`-based derivation anyway. Proposal: try `Expr.summon[Mirror.Of[T]]` first (cheap — 0.17ms avg). If a Mirror exists, derive directly and skip `summonIgnoring`. If no Mirror (primitives, opaque types), fall back to `summonIgnoring`. **Key concern**: user-provided instances for types that also have Mirrors would be silently ignored. Approaches to investigate (in order of safety): **(a)** Derive first, then `summonIgnoring` as a "user override" check — still 660 calls but derivation is cached so the cost shifts to correctness-only checks that could be batched or deferred. **(b)** Only skip `summonIgnoring` for types whose `Symbol` is defined in the same source file / compilation unit as the root type being derived — external types keep the full summon-first path since those are more likely to have user-provided instances. **(c)** Full mirror-first: skip `summonIgnoring` unconditionally when a Mirror exists — fastest but changes semantics (user instances ignored for Mirror types). Need to verify: does circe-generic even support user overrides for nested types in auto mode? If not, option (c) is safe. Worst-case offenders: `Encoder[Sprint]` 61.7ms (single call), `Paginated[X]` ~5.5ms each. Estimated savings: 600–800ms (60–80% of summonIgnoring time) on auto derivation.
