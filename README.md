...
+ mvn"io.github.nguyenyou::circe-sanely-auto:0.11.0"
```
...
- [ ] **(P1) Single-pass auto derivation** — `sanely.auto` still derives `Encoder[A]` and `Decoder[A]` via separate inline givens, so a codebase that both encodes and decodes the same type pays for two macro expansions and two per-expansion caches. Investigate routing auto derivation through one shared `Codec.AsObject[A]` expansion and projecting encoder/decoder views from it without breaking circe compatibility.
- [ ] **(P1) Eliminate redundant sub-trait `summonIgnoring`** — sub-trait detection (line 74-75) calls `Expr.summonIgnoring[Encoder[t]]` to check if a user-provided encoder exists, but `resolveOneEncoder` already resolved an encoder for this variant. Pass the resolution source (summoned vs derived) to avoid the redundant implicit search. Saves up to 336 calls.
