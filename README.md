...
- [x] **(P1) Mirror-first resolution: skip `summonIgnoring` for derivable types** — `summonIgnoring` was 51% of macro time (1031ms/2028ms, 660 calls, avg 1.56ms). Now: try `Expr.summon[Mirror.Of[T]]` first (0.22ms avg). If a Mirror exists, check `MacroUtils.mayHaveExternalInstances` — if the type has companion givens, is from `scala.*`/`java.*`/`io.circe.*`, or is from a different source file, fall through to `summonIgnoring`. Otherwise, derive directly via Mirror. Results: summonIgnoring calls 660→66 (-90%), summonIgnoring time 1031→634ms (-38%), total macro time 2028→1868ms (-8%). Benchmark: auto 2.44→2.01s (-18%), 3.69x faster than circe-generic. **Limitation**: local-scope givens (defined in method body, not companion) for same-file types are not detected — instances must be in the companion object.
