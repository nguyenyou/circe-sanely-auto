...
- [x] **Deeply nested null propagation** — Verify that a decode error deep in a nested structure (3+ levels) propagates as an exception, not silently returning nullValue (all-null object). Tests should prove: (a) `null` at field level → field gets nullValue (correct), (b) malformed JSON at nested level → exception propagates to caller, (c) no silent data corruption.
